...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        # Have a habit of leaving these in at this point

        stateAndProbability = self.mdp.getTransitionStatesAndProbs(state, action)
        QValue = 0
        for nextState, probability in stateAndProbability:
    ...
    def computeActionFromValues(self, state):
        ...
        # Nothing for a terminal state
        if self.mdp.isTerminal(state):
            return None

        # Getting all possible actions for current state
        actions = self.mdp.getPossibleActions(state)
        # the best action is the one with the highest resulting Q
